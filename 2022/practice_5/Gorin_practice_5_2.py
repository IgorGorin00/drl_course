...
class DQN():
    ...
    def training_step(self, state, action, reward, done, next_state, gamma, model):
    ...
    for epoch in range(epochs):
        ...
        for traj_i in tqdm(range(traj_per_epoch)):
            ...
            for _ in range(trajectory_len):
                ...
                loss = agent.training_step(state, action, reward, done, next_state, gamma, training_model)
...
def train_dqn_soft_update(epochs, traj_per_epoch, 
                          agent, target_model, training_model, 
                          env, trajectory_len, action_n, 
...
history_soft_update = train_dqn_soft_update(epochs, traj_per_epoch, 
                                            agent, target_model, training_model,
                                            env, trajectory_len, action_n, batch_size, gamma, tau, lr, opt_f=opt_f)
...
with plt.style.context('ggplot'):
    ...
    plt.plot(history_soft_update['train_rewards'], label='Epoch mean reward (train)')
    plt.plot(history_soft_update['val_rewards'], label='Epoch val rewards (real model)')
...
with plt.style.context('ggplot'):
    ...
    plt.plot(history_soft_update['train_rewards'], label='Epoch mean reward (train), soft update', color='blue')
    plt.plot(history_soft_update['val_rewards'], label='Epoch val rewards (real model), soft update', linestyle='-.', color='blue')

    plt.plot(history_hard_update['train_rewards'], label='Epoch mean reward (train), hard update', color='red')
    plt.plot(history_hard_update['val_rewards'], label='Epoch val rewards (real model), hard update', linestyle='-.', color='red')
...
for checkpoint in history_hard_update['checkpoints']:

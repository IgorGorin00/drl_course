...
class CEM():
    def __init__(self, state_n, action_n, possible_actions):
        ...
        self.policy = possible_actions          # matrix that presents probs of choosing an action for particular state
...
def get_possible_actions(env, state_n, action_n):
    ...
    for state in range(state_n):
        possible_actions[state] =\
                env.action_mask(state) / sum(env.action_mask(state))
...
def get_elite_trajectories(trajectories, q):
    ...
    return np.mean(rewards), [t for t in trajectories if t['max_reward'] > q_value]
...
def train(n_epochs, n_traj_per_epoch, env, agent, q=0.9):
    ...
    for epoch in range(n_epochs):
        ...
        trajectories = [get_trajectory(env, agent) for _ in range(n_traj_per_epoch)]

        mean_reward, elite_trajectories = get_elite_trajectories(trajectories, q)
